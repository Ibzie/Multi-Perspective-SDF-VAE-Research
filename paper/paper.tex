...
%%%%%%%%% ABSTRACT
\begin{abstract}
Variational Autoencoders (VAEs) suffer from two fundamental limitations: latent representations lack interpretability, and uncertainty estimates are unreliable. We introduce Multi-Perspective SDF-VAE, a novel architecture that addresses both challenges through geometric inductive biases. The model employs multiple observer networks that learn complementary signed distance function (SDF) representations of the data manifold, aggregated via SDF-based confidence weighting that directly reflects geometric proximity to the manifold. A universal light source projection ensures consistent feature illumination, while curriculum learning stabilizes the multi-objective optimization combining reconstruction, KL divergence, SDF consistency, Eikonal regularization, and observer diversity. Experiments on CelebA (162k face images) demonstrate exceptional improvements over standard VAEs: 358$\times$ more stable latent variance (LogVar Std: 0.004 vs 1.42), near-perfect generalization (test/train ratio: 0.9996 vs 1.004), and maximum observer diversity (entropy: 1.609/1.609 max). Cross-validation on Fashion-MNIST confirms 3.6$\times$ better uncertainty calibration (ECE: 0.129 vs 0.460) and near-perfect out-of-distribution detection (AUROC: 0.992). The learned geometry exhibits SDF values near zero (mean: 0.011) with perfectly balanced observer utilization, providing interpretable representations for safety-critical applications in medical imaging, active learning, and trustworthy AI systems.
\end{abstract}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/sdf_concept.pdf}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/vae_comparison.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{figures/light_source.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{figures/weighting_comparison.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/reparameterization.pdf}
...
\begin{align}
&\text{Conv}_1: 3 \to 32 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 32 \times 32 \nonumber \\
&\text{Conv}_2: 32 \to 64 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 16 \times 16 \nonumber \\
&\text{Conv}_3: 64 \to 128 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 8 \times 8 \nonumber \\
&\text{Conv}_4: 128 \to 256 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 4 \times 4 \nonumber \\
&\text{Flatten} + \text{Linear}: 256 \times 4 \times 4 = 4096 \to 512 \text{ dimensions} \to f \in \mathbb{R}^{512} \nonumber
\end{align}
...
\begin{align}
&\text{Linear}: z \in \mathbb{R}^{128} \to 4096 \to \text{Reshape to } 256 \times 4 \times 4 \nonumber \\
&\text{TransposeConv}_1: 256 \to 128 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 8 \times 8 \nonumber \\
&\text{TransposeConv}_2: 128 \to 64 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 16 \times 16 \nonumber \\
&\text{TransposeConv}_3: 64 \to 32 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 32 \times 32 \nonumber \\
&\text{TransposeConv}_4: 32 \to 3 \text{ channels, kernel } 4 \times 4 \text{, stride } 2 \text{, padding } 1 \to 64 \times 64 \nonumber \\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\columnwidth]{figures/kl_divergence.pdf}
...
Gradients are computed via automatic differentiation with respect to observer inputs. This constraint ensures SDFs behave as proper distance functions, critical for reliable uncertainty estimation.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/eikonal.pdf}
...
\begin{table}[htbp]
\centering
\caption{\textbf{Cross-Dataset Evaluation of SDF-VAE.} Variance stability (LogVar Std), generalization ratio, SDF manifold proximity, and observer entropy across two image domains. Lower LogVar Std is better; Gen. Ratio closer to 1.0 is better; SDF Mean closer to 0 is better; higher Entropy indicates greater observer diversity.}
\label{tab:cross_dataset}
\begin{tabular}{lcccc}
\toprule
\textbf{Dataset} & \textbf{LogVar Std} & \textbf{Gen. Ratio} & \textbf{SDF Mean} & \textbf{Entropy} \\
\midrule
Fashion-MNIST & \textbf{0.004} & 0.935 & 0.009 & 1.608/1.609 \\
CelebA & \textbf{0.004} & \textbf{0.9996} & 0.011 & \textbf{1.609/1.609} \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:latent_tsne} shows t-SNE visualizations of latent codes from all three models trained on CelebA, revealing critical insights into the learned latent geometry. SDF-VAE demonstrates the most structured manifold organization compared to baseline VAEs.

\textbf{Manifold Structure Analysis}. CelebA lacks discrete class labels; instead, facial variation is continuous across attributes (identity, expression, age, hair). This makes the latent space analysis more challenging but also more revealing: a well-structured latent space must capture continuous attribute variation smoothly. SDF-VAE's t-SNE shows a compact, well-organized distribution with smooth density gradients, whereas Vanilla VAE tends toward more scattered and irregular arrangement. This structural difference directly reflects the Eikonal regularization: enforcing $\|\nabla \text{SDF}\| = 1$ creates Lipschitz-continuous distance functions that induce smooth, continuous latent manifolds.

\textbf{Connection to Geometric Constraints}. The smooth, continuous manifold structure validates the theoretical prediction in Section~\ref{sec:curriculum}: geometric constraints that stabilize SDF values simultaneously create organized latent representations. The absence of sharp discontinuities or isolated outlier clusters is consistent with the Lipschitz bound derived in Appendix~\ref{app:theory}.

\textbf{Implication for Interpretability}. A well-organized continuous manifold implies that latent interpolations produce semantically coherent face transitions (Section~\ref{sec:qualitative}), and that geometric proximity in latent space reflects genuine similarity in face attributes. Observers collectively learn to organize facial features despite no explicit supervision, demonstrating that geometric interpretability (via SDFs) naturally structures the latent manifold.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/latent_tsne.pdf}
\caption{t-SNE visualizations of latent spaces from all three models trained on CelebA. SDF-VAE (left) shows the most compact and continuously organized manifold structure, reflecting the smooth geometry induced by Eikonal regularization. Vanilla VAE (center) and $\beta$-VAE (right) exhibit less organized distributions. Each model encodes 5,000 test face images.}
...
\caption{Latent variance stability comparison across models trained on CelebA. SDF-VAE exhibits exceptional stability (Std: 0.004), 358$\times$ better than Vanilla VAE (Std: 1.42), demonstrating calibrated uncertainty estimation as an emergent property of geometric constraints.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/reconstruction_comparison.png}
...
\textbf{Key Insight.} While ensemble methods aggregate uncertainty across multiple models and GP-based methods model latent distributions, SDF-VAE achieves calibrated uncertainty as an \textit{emergent property} of geometric constraints. This architectural inductive bias provides uncertainty quantification without the computational overhead of ensembling or GP inference, making it more practical for real-time applications like medical diagnosis or active learning.
...
The 358$\times$ improvement in variance stability on CelebA (LogVar Std: 0.004 vs 1.421) was not explicitly optimized but emerges from geometric constraints. We provide theoretical intuition for why this occurs.
